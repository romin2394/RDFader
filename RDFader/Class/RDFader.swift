...
@IBDesignable open class RDFader: UIControl {
    ...
    /// The current (or starting) value for the slider
    @IBInspectable open private(set) var step: Int = 0 {
        didSet {
            self.addDevider(step: step)
        }
    }
	...
	private let renderer = RDFaderLayerRenderer()
    ...
    func addDevider(step: Int){
        var x = 0
        for i in 0..<step {
            if i != 0 {
            let view = UIView(frame: CGRect(x: x, y: 10, width: 1, height: Int(self.bounds.height)-20))
            view.backgroundColor = UIColor.white
            self.addSubview(view)
            }
            x += Int(self.bounds.width) / step
        }
    }
		...
		if value == minimum && valueChange < 0 {
			...
			if #available(iOS 10.0, *) {
				...
				case .cancelled, .ended, .failed:
                    ...
                    if self.step != 0 {
                        let roundValue = roundf(newValue/(self.maximum-self.minimum)*Float(self.step))*(self.maximum-self.minimum)/Float(self.step)
                        setValue(roundValue, animated: false)
                    } else {
                        setValue(newValue, animated: false)
                    }
				default:
	...
	@objc func didTap(sender: UITapGestureRecognizer) {
		...
		if sender.state == .ended {
			...
			let tappedValue = Float(valueForPosition(tapLocation))
            if self.step != 0 {
                let roundValue = roundf(tappedValue/(self.maximum-self.minimum)*Float(self.step))*(self.maximum-self.minimum)/Float(self.step)
                setValue(roundValue, animated: true)
            } else {
